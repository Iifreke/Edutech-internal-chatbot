...
/**
 * Robust extraction of text runs from binary files (e.g. legacy Word .doc files)
 */
function extractReadableStrings(buffer) {
  ...
  return matches
    .filter((s) => !/^[0-9a-fA-F]{8,}$/.test(s) && !/^[\W_]+$/.test(s))
    .join(' ')
    .replace(/\s+/g, ' ')
    .trim();
}
...
export async function parseFile(buffer, filename) {
  ...
  switch (ext) {
    ...
    default:
      throw new Error(`Unsupported file type: .${ext}. Please upload PDF, DOCX, DOC, TXT, MD, XLSX, or CSV.`);
  }
}
